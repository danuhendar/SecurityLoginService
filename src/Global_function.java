...
public class Global_function {
   ...
   public void PublishMessageDocumenter(boolean is_dokumentasi,int profil,String topic,byte[] content,int counter,String plain_text_res_message,int qos){
       try {
           Date HariSekarang = new Date();
           //-- sesi koneksi broker --//
           String res_broker_primary       		= "";
           String res_username_primary     		= "";
           String res_password_primary     		= "";
           boolean res_clean_session_primary 	= false;
           int res_keep_alive_primary		 	= 60;
           boolean res_autoreconnect_primary 	= false;
           
           if(profil == 1) {
        	   res_broker_primary		   =  en.getIp_broker_primary()+":"+en.getPort_broker_primary();
        	   res_username_primary		   =  en.getUsername_broker_primary();	
        	   res_password_primary		   =  en.getPassword_broker_primary();
        	   res_clean_session_primary   =  Boolean.parseBoolean(en.getCleansession_primary());
        	   res_keep_alive_primary	   =  60;// Integer.parseInt(en.getKeepalive_primary());
        	   res_autoreconnect_primary   =  Boolean.parseBoolean(en.getReconnect_primary());
        	   
           }else {
        	   res_broker_primary		   =  en.getIp_broker_secondary()+":"+en.getPort_broker_secondary(); 
        	   res_username_primary		   =  en.getUsername_broker_secondary();	
        	   res_password_primary		   =  en.getPassword_broker_secondary();
        	   res_clean_session_primary   =  Boolean.parseBoolean(en.getCleansession_secondary());
        	   res_keep_alive_primary	   =  Integer.parseInt(en.getKeepalive_secondary());
        	   res_autoreconnect_primary   =  Boolean.parseBoolean(en.getReconnect_secondary());
           }
           ...
           try{
               ...
               options.setCleanSession(res_clean_session_primary);
               options.setKeepAliveInterval(res_keep_alive_primary);
               options.setAutomaticReconnect(res_autoreconnect_primary);
               ...
               PrintMessage2("SEND "+profil+" > "+Parser_TASK, counter, "json", topic, Parser_TASK, Parser_FROM, Parser_TO, HariSekarang, HariSekarang);
           }catch(Exception exc){
              exc.printStackTrace();
           }
       } catch (Exception e) {
 ...
 public void InsertDocument(String nama_database,String table,Document doc){
   ...
   public boolean create_table_mysql(String nama_table) {
    ...
    public String CreateMessageNested(String IN_TASK,
					            String IN_ID,
					            String IN_SOURCE,
					            JSONArray IN_COMMAND,
					            String IN_OTP,
					            String IN_TANGGAL_JAM,
					            String IN_VERSI,
					            String IN_HASIL,
...
//     
    public String get_tanggal_curdate(){
     ...
     public void WriteLog(String res_nama_file,String content_after,boolean append_content){
       
    	 	String read = "";
        	
        	boolean tulis_log = Boolean.parseBoolean(en.getTulisLog());
        	//System.out.println("tulis_log : "+tulis_log);
            if(tulis_log) {
            	try {
                    String tanggal = get_tanggal_curdate();
                    ...
                    int jam = Calendar.getInstance().get(Calendar.HOUR_OF_DAY);
                    	...
                    	String nama_file = res_nama_file+"_"+tanggal+"_"+jam+".txt";
                        ...
                        if(append_content == true){
                                ...
                                //System.out.println("SUKSES TULIS LOG 1");
                            
                        }else{
                             WriteFile(nama_file,"",content_after+"\n",append_content);
                             //System.out.println("SUKSES TULIS LOG 2");
                        }
            	...
            	//System.out.println("INFO : "+tulis_log);
            }
...
/* Decryption Method */
    ...
    public String get_pass_sql_toko() {
    	 String res = "";
         try{
             res = GetTransReport("SELECT GROUP_CONCAT(PASS) AS PASS_SQL FROM m_pass_sql WHERE DB LIKE 'pos%'", 1, true);
    ...
    public String get_pass_sql_dbl() {
   	 String res = "";
        try{
            res = GetTransReport("SELECT GROUP_CONCAT(PASS) AS PASS_SQL FROM m_pass_sql WHERE DB LIKE 'dbl%'", 1, true);
